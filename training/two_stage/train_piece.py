#!/usr/bin/env python3
"""Train the second-stage piece classifier on per-cell crops.

Uses BoardDataset (from the autoresearch harness) for image loading + corners,
wraps it with PieceCropDataset to extract one (patch, label) per piece cell.
"""
...
def main(time_budget_s=3600.0, batch_size=64, lr=3e-4, weight_decay=0.01):
    ...
    # Approx total steps = epochs * (train/bs). For 1hr at GTX 1070 with 64 bs, expect ~20-30 epochs.
    total_steps = max(1000, int(time_budget_s * 8))
    gamma = 0.02 ** (1.0 / total_steps)
    ...
    while time.time() - start < time_budget_s:
        for x, y in train_loader:
            ...
            if time.time() - start >= time_budget_s:
                break
            if time.time() - last_eval >= 300.0:
                acc = evaluate(model, val_loader, device)
                elapsed = time.time() - start
                print(f"step={step:>6} elapsed={elapsed:.0f}s loss={loss.item():.4f} val_acc={acc:.4f} lr={sched.get_last_lr()[0]:.2e}")
                if acc > best_val:
                    best_val = acc
    ...
    print(f"\nFINAL val_acc={acc:.4f}  best={best_val:.4f}")

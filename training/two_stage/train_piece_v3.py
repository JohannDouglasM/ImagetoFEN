#!/usr/bin/env python3
"""Train piece classifier v3: aux color + type heads on top of v2 setup.

Joint loss: piece_ce + 0.3 * (color_ce + type_ce). Targets the dominant
residual error in v2's confusion matrix — k↔q, B↔P, etc — by giving the
backbone two easier supervisory signals (color and type) that decompose
the 12-way decision into structurally consistent sub-decisions.
"""
...
def main(time_budget_s=3600.0, batch_size=128, lr=5e-4, weight_decay=0.01, eval_every_s=180.0):
    ...
    from piece_model import build_piece_model_aux, PIECE_TO_COLOR, PIECE_TO_TYPE
    ...
    model = build_piece_model_aux().to(device)
    color_map = torch.tensor(PIECE_TO_COLOR, device=device, dtype=torch.long)
    type_map = torch.tensor(PIECE_TO_TYPE, device=device, dtype=torch.long)
    ...
    while time.time() - start < time_budget_s:
        for x, y in train_loader:
            ...
            type_y = type_map[y]
            out = model(x)
            piece_loss = F.cross_entropy(out["piece"], y, label_smoothing=0.05)
            color_loss = F.cross_entropy(out["color"], color_y)
            type_loss = F.cross_entropy(out["type"], type_y)
            loss = piece_loss + AUX_WEIGHT * (color_loss + type_loss)
            opt.zero_grad()
            loss.backward()
            opt.step()
            sched.step()
            ...
            if time.time() - last_eval >= eval_every_s:
                ...
                print(f"step={step:>6} elapsed={elapsed:.0f}s loss={loss.item():.4f} "
                      f"piece={piece_loss.item():.4f} color={color_loss.item():.4f} "
                      f"type={type_loss.item():.4f} val_acc={acc:.4f} "
...
def evaluate(model, loader, device):
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            preds = model(x)["piece"].argmax(dim=1)
...
def evaluate_confusion(model, loader, device):
    ...
    with torch.no_grad():
        for x, y in loader:
            x = x.to(device)
            preds = model(x)["piece"].argmax(dim=1).cpu().numpy()
